...
impl AsSExpr for Expression<'_> {
    fn as_sexpr(&self) -> SExpr {
        match self {
            ...
            Expression::Nullptr => SExpr::string("nullptr"),
            ...
            Expression::PtrCmp { lhs, kind, rhs } =>
                SExpr::new(format!("ptr-{}", kind.str())).lines([lhs, rhs]),
            ...
            Expression::CompoundLiteral { decl } => SExpr::new("compound-literal").inherit(decl),
        }
    }
}
